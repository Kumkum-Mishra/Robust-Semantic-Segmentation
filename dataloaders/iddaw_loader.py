...
class IDDAWDataset(Dataset):
    ...
    def __getitem__(self, idx):
        image = Image.open(self.image_paths[idx]).convert("RGB").resize(self.image_size)
        label = Image.open(self.label_paths[idx]).resize(self.image_size)

        image = T.ToTensor()(image)
        label = T.PILToTensor()(label).squeeze(0).long()
        return image, label
